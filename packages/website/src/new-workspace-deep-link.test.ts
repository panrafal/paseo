...
describe("buildNewWorkspaceDeepLink", () => {
  ...
  it("returns a temporary redirect for website requests", () => {
    const response = createNewWorkspaceRedirect("https://paseo.sh/new?serverId=host-1&dir=%2Frepo");

    expect(response.status).toBe(307);
    expect(response.headers.get("Location")).toBe("paseo://new?serverId=host-1&dir=%2Frepo");
  });
});
