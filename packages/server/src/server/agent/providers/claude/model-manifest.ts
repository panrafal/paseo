...
export function normalizeClaudeRuntimeModelId(value: string | null | undefined): string | null {
  ...
  // These matches are unanchored so a gateway prefix can precede the model. The major-minor
  // form has to be tried first: a single-segment match against "claude-opus-5-5" stops at
  // "claude-opus-5", which is a catalog entry in its own right, and would resolve Opus 5.5 to
  // Opus 5. A dated single-segment ID is safe here because its date is never a catalog minor.
  const runtimeMatch = trimmed.match(
