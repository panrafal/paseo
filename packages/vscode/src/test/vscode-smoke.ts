...
export async function run(): Promise<void> {
  ...
  // The editor context menu entry is only as real as the command behind it.
  const commands = await vscode.commands.getCommands(true);
  assert.ok(
    commands.includes("paseo.sendToPaseo"),
    "activation registers the Send to Paseo command",
  );
  ...
  // Plugin client bundles are evaluated from source; without 'unsafe-eval' every plugin fails.
  assert.match(cspMatch[1], /script-src [^;]*'unsafe-eval'/);
