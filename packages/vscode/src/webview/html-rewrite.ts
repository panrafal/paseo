...
function buildCsp(cspSource: string, nonce: string): string {
  return [
    ...
    // 'unsafe-eval' is what plugins run on. The app evaluates each plugin's client bundle from
    // source (packages/app/src/plugins/evaluate.ts) because that is the one loader that works on
    // both Hermes and the web; without it every installed plugin fails in the webview. The nonce
    // still blocks injected <script> tags, which is the guard that matters for rendered agent
    // output, and 'unsafe-eval' does not weaken it.
