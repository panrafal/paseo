...
This checkout is `panrafal`, a personal fork of `getpaseo/paseo`. It is an
integration branch, rebuilt from scratch on every sync as `upstream/main` plus
a list of patch branches. **Nothing committed directly to `panrafal` survives.**

| Want to...                            | Do this                                                    |
| ------------------------------------- | ---------------------------------------------------------- |
| Change Paseo itself                   | Branch off `upstream/main`, then add it to `fork/branches` |
| Change how the fork builds or ships   | Commit to `panrafal-dist`                                  |
| Change the fork scripts               | Commit to `fork-tooling`                                   |
...
- **NEVER commit to `panrafal`.** It is regenerated by `fork/sync.sh` and your commit is lost. See [fork/README.md](fork/README.md) for where the change belongs.
