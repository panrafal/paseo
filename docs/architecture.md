...
- The public key authenticates only the daemon. With `daemon.relay.deviceAuth` on, the daemon announces `relayAuth` and attaches no session until the client proves itself inside E2EE with a one-time pairing token, a device credential, or the daemon password (`server/relay-auth/`, `client/daemon-client-relay-e2ee-transport.ts`). The app keeps tokens and credentials outside the host registry (`app/src/runtime/relay-auth.ts`), because a changed connection object reconnects the host.
...
**Multi-window (hybrid land-on model).** `createWindow()` in `main.ts` is reusable: `⌘⇧N`/File→New Window, relaunching the app without a deep link (`second-instance`), and the sidebar "Open in new window" action each open a fresh `BrowserWindow`. Every window shows the full sidebar — there is no per-window project ownership or filtering. "Land on a project" is delivered by a per-`webContents` `PendingOpenProjectStore`: each window pulls its own pending project path on mount (`paseo:get-pending-open-project`) and runs the normal open-project flow, identical to a CLI `paseo <path>` launch.

**Deep links reuse a window.** `paseo://h/<server-id>/agent/<agent-id>` and `paseo://new?…` focus the current window and reach its renderer through `DesktopNavigationInbox`, which holds the target until that renderer reports ready. A window opens only when none exists. On macOS a link that starts a new process arrives through `open-url` before `ready`, never through argv, so the single-instance lock waits for `ready` and forwards the link as lock data. Without that, the running app sees a bare relaunch and opens a window at the last workspace.
