...
Permission updates exchange these semantic permissions directly. Hub enrollment still speaks legacy scopes. Hub's enrollment body is strict and knows only `scopes`, so the daemon sends no permissions. Hub answers with its fixed `hub.execution.*` grant, which `permissionsForLegacyHubScopes` in `packages/server/src/server/authorization/index.ts` maps to `hub.execute`. The daemon keeps only what it requested as local authority; a grant narrower than the request fails enrollment. Legacy persisted Hub relationships that contain `hub.execution.*` migrate once to `hub.execute` when the daemon loads them; new relationships never persist or emit transport scopes as authority.
