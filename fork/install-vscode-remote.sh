...
# windows without it. fork/deploy.sh copies this next to the .vsix and runs
# it for you. It needs no sudo and nothing from fork/config.sh.
#
...
# "Developer: Reload Window". Every product gets one line, `installed:` or
# `skipped:`, which deploy.sh reads for its summary.
...
for product in "VS Code:$HOME/.vscode-server:code-server" "Cursor:$HOME/.cursor-server:cursor-server"; do
  ...
  if [ ! -d "$root" ]; then
    echo "skipped: $name has never connected to this host ($root is missing)"
    continue
  fi
  ...
  if [ -z "$cli" ]; then
    echo "skipped: $name has no server binary under $root"
    continue
  fi
  "$cli" --install-extension "$vsix" --force
  echo "installed: $name server on $(hostname) ($cli)"
